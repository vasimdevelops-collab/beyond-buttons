...
async function readMongoCatalog() {
  ...
  try {
    ...
    const { CategoryModel, ProductModel, SettingsModel } = await import("@/lib/database/models");

    await bootstrapDatabase();

    const [categoryDocs, productDocs, settingsDoc] = await Promise.all([
      CategoryModel.find({ visibility: { $ne: false } }).sort({ order: 1 }).lean().exec(),
      ProductModel.find({ status: "active" }).sort({ updatedAt: -1 }).lean().exec(),
...
/**
 * Database-backed product list when Mongo is reachable; JSON catalog only as a
 * last-resort fallback when the database itself is unavailable. An empty DB
 * (e.g. every product deleted in the admin panel) yields an empty list so the
 * storefront stays in sync with the admin catalog.
 */
function productSource(mongo) {
  if (!mongo) return PRODUCT_RECORDS.map(normalizeProduct);
  return mongo.products || [];
}

export async function getProductsServer() {
  const mongo = await readMongoCatalog();
  return productSource(mongo);
}
...
export async function getProductBySlugServer(slug) {
  const mongo = await readMongoCatalog();
  const source = productSource(mongo);
