...
async function ensureCredential() {
  try {
    ...
    // Fallback when MongoDB is not available - use .env values directly
    const email = String(process.env.ADMIN_EMAIL || "").trim();
    const password = String(process.env.ADMIN_PASSWORD || "");
    if (!email || !password) {
      throw new Error("ADMIN_EMAIL and ADMIN_PASSWORD must be set in .env");
...
export async function verifyAdminCredentials(email, password) {
  try {
    ...
    const emailMatches = String(email || "").trim().toLowerCase() === envEmail;
    if (!emailMatches) return false;
    const salt = crypto.randomBytes(16).toString("hex");
    const expectedHash = hashPassword(envPassword, salt);
    return timingSafeEqualHex(hashPassword(password, salt), expectedHash);
  }
}
