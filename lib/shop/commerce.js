...
export function CartProvider({ children }) {
  ...
  const placeOrderWithPayment = useCallback(
    async () => {
      ...
      // ── 1. Create the order record FIRST (paymentStatus "pending").
      // The Razorpay webhook flips it to "paid"/"failed" after verification.
      // Creating it first lets us pass our internal order id into the Razorpay
      // order notes so the webhook can match the payment to this order.
      const previousItems = items;
      const order = await placeOrder();

      // Amount is already in rupees; razorpay.js expects rupees and converts to paise internally.
      // Do NOT multiply by 100 here — that would double-convert and exceed Razorpay's max amount.
      ...
      let razorpayOrder;
      try {
        razorpayOrder = await paymentAdapter.createRazorpayOrder(amount, "INR", receipt, notes);
      } catch (error) {
        // Gateway init failed — restore the cart so the user can retry.
        itemsStore.set(previousItems);
        throw error;
      }
